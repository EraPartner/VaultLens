...
1. `lint` + `index` + `qmd update` (offline, host-native pre-check), then a
   bounded `qmd embed` pass on AC power. Embedding saves progress and resumes on
   later nights until the semantic index is current.
...
**Scheduled (in `build_steps`), in run order:** lint, index, qmd update, qmd embed, ingest, contradict,
