...
> **Privacy / isolation.** This box mounts **only this repo** (`~/Code/VaultLens`,
> RW) plus a sanitized `~/.claude` stage. It does **not** mount the live Brain vault
> or any iCloud/personal data, and `qmd` starts with an **empty index** (it does not
...
`python3 tools/wiki.py lint`, and search with `qmd`. No published service.
...
```sh
vaultlens-claude --dangerously-skip-permissions
```

To drop into a shell instead of Claude:

```sh
container exec -it --user dev vaultlens-dev bash
```
...
```sh
VAULTLENS_REBUILD=1 vaultlens-claude --dangerously-skip-permissions
```

By default the launcher powers the container's VM down on session exit so it stops
pinning its 4 GB. Set `VAULTLENS_STOP_ON_EXIT=0` to keep it warm for instant reuse.
...
2. **`iptables` egress lock** (`init-firewall.sh`): default-deny, then only
   ...
   rate-limited-logged (`dmesg | grep vaultlens-deny`).
...
`nodejs.org`, `malware-list.aikido.dev`, …) plus VaultLens's own
`allowlist.extra.txt`.
...
To change the allowlist, edit VaultLens's `allowlist.extra.txt` (or the shared
`base-allowlist.txt`), re-run `LockBox/sync.sh` to regenerate
`.devcontainer/allowlist.txt`, and **rebuild** (`VAULTLENS_REBUILD=1
vaultlens-claude`) so it's re-baked into the image. `dev` can't re-run the
...
`dmesg | grep vaultlens-deny` catches proxy-bypass attempts but needs root.
...
| ----------------------------------------- | ------------------------------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `$PROJECT_ROOT` (host)                    | `/workspaces/VaultLens`               | bind **RW**  | The VaultLens repo — edits appear on the host immediately                                                                                                |
| `$PROJECT_ROOT/.git` (host)               | `/workspaces/VaultLens/.git`          | bind **RO**  | Git history is readable but immutable from inside (see "Git")                                                                                           |
| `.devcontainer` (host)                    | `/workspaces/VaultLens/.devcontainer` | bind **RO**  | Overlay on the rw workspace so the sandbox config + host launcher can't be rewritten from inside (see Safety note)                                      |
| `vaultlens-claude`                         | `/home/dev/.claude`                  | named volume | Container's writable Claude config — seeded from the sanitized stage on first create                                                                    |
| `~/.claude-sandbox/stage/vaultlens` (host) | `/home/dev/.claude-stage`            | bind **RO**  | Sanitized staging copy the launcher produces (secrets + `hooks`/`mcpServers`/`enabledPlugins` stripped). The raw host `~/.claude` is **never** mounted. |
| (container fs)                            | `/home/dev/.claude.json`             | regular file | Container's writable global config, seeded from `…/claude.json` in the stage                                                                            |

`vaultlens-claude` is a **native apple/container named volume** — the root
...
The fish function `vaultlens-claude-sync` propagates Claude config (settings,
...
```sh
vaultlens-claude-sync pull     # refresh container from host (also auto-runs on launch)
vaultlens-claude-sync push     # propagate container changes back to host
vaultlens-claude-sync status   # show what differs
```
...
The `vaultlens-claude` launcher re-stages a sanitized copy of host
`~/.claude` into `~/.claude-sandbox/stage/vaultlens` on every invocation, and
...
The reverse (container → host) runs automatically. The `vaultlens-claude`
...
security add-generic-password \
  -s "vaultlens-claude-code-token" \
  -a "$USER" \
  -w   # prompts you to paste the token (won't echo)
```

That's it. The `vaultlens-claude` launcher now does
`security find-generic-password -s vaultlens-claude-code-token -w` on
...
```sh
security delete-generic-password -s "vaultlens-claude-code-token"
...
read-write at `/workspaces/VaultLens` so the agent can edit source — but that
